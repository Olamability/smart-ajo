...
export async function ensureUserProfile(
  ...
  if (profile) {
    logger.log('ensureUserProfile: Profile already exists');
    return true;
  }

  logger.log('ensureUserProfile: Profile does not exist, creating...');
  ...
  try {
    logger.log('ensureUserProfile: Attempting profile creation via RPC function');
    ...
    if (rpcError && !isDuplicateError(rpcError)) {
      ...
      if (rpcError.message?.includes('Could not find the function') ||
        ...
        logger.warn('ensureUserProfile: RPC function not found, trying fallback');
    ...
    logger.log('ensureUserProfile: Profile created successfully via RPC');
    return true;
  } catch (rpcError) {
    logger.warn('ensureUserProfile: RPC method failed, trying direct insert');
    lastError = rpcError instanceof Error ? rpcError : new Error(String(rpcError));
  }

  // Method 2: Try direct insert as fallback
  try {
    logger.log('ensureUserProfile: Attempting profile creation via direct insert');
    ...
    logger.log('ensureUserProfile: Profile created successfully via direct insert');
    return true;
  } catch (insertError) {
